"""
    ally model version 2
    当遇到正确图像时，计算两个loss,并且结果求平均
    当遇到错误图像时，只计算分类loss
"""
...
@Register(name="internvl2_ally_model_v2")
class InternVL2AllyModelV2(nn.Module):
    def __init__(
        self,
        model_path,
        use_backbone_lora=0,
        ...
        self.model: InternVLChatModelForAlly = InternVLChatModelForAlly.from_pretrained(
            model_path,
            config=config,
    ...
    def forward(
        ...
        if vqa_loss is not None and cls_loss is not None:
            ...
            cls_label = cls_label.view(B).to(dtype=dtype)
            ...
            # 采用0.5 对0.5的权重
            # vqa_factor = 0.5 * cls_label  # 正确图像0.5,错误图像0 
            # cls_factor = torch.where(cls_label == 0, 1, 0.5) # 正确图像0.5,错误图像1
            # loss = cls_factor * cls_loss + vqa_factor * vqa_loss
            # loss = loss.mean()
