...
@Register(name="internvl2_classify_model_v2")
class InternVL2ClassifyModelV2(nn.Module):
    def __init__(
        self,
        model_path,
        use_backbone_lora=0,
    ...
    def reduce_hidden_states(
        ...
        if self.reduction == "mean":
            # 需要把pdding的部分去掉
            return self.get_mean_hidden_states(hidden_states, attention_mask)
        elif self.reduction == "-1":
            return self.get_last_one_hidden_statesv2(
                hidden_states, input_ids, padding_on_left=True, wanted_position=-1
            )
        elif self.reduction == "-2":
            return self.get_last_one_hidden_statesv2(
                hidden_states, input_ids, padding_on_left=True, wanted_position=-2
            )
        elif self.reduction == "0":
            return self.get_last_one_hidden_statesv2(
                hidden_states, input_ids, padding_on_left=True, wanted_position=0
            )
        else:
            raise ValueError(f"reduction {self.reduction} is not supported")

    def get_mean_hidden_states(self, hidden_states, attention_mask):
        """
        Args:
            hidden_states: torch.Tensor, [B, L, D]
        ...
        # [B, D] / [B, 1]
        return hidden_states.sum(dim=1) / attention_mask.sum(dim=1).unsqueeze(-1)
    ...
    def get_last_one_hidden_states(
        ...
        if padding_on_left:
            return hidden_states[:, wanted_position, :].view(bsz, -1)
        else:
            ...
            for i in range(bsz):
                ...
                last_idx.append(t[0].min() - wanted_position)
    ...
    def get_last_one_hidden_statesv2(
        self, hidden_states, input_ids, padding_on_left: bool, wanted_position: int = -1
    ):
        """
        Args:
            hidden_states: torch.Tensor, [B, L, D]
            input_ids: torch.Tensor, [B, L]
            padding_on_left: bool
            wanted_position: int
        """
        bsz, _, _ = hidden_states.size()
        if wanted_position < 0:
            return self.get_last_one_hidden_states(
                hidden_states,
                input_ids=input_ids,
                padding_on_left=padding_on_left,
            ...
            # position >= 0, 根据padding的不同需要不同的处理
            if not padding_on_left:
                return hidden_states[:, wanted_position, :].view(bsz, -1)
            else:
                device = input_ids.device
                # padding on left
                ...
                # [|, 1, 1, 2, 3, 4, 5]
                temp_pad = (
                    torch.tensor([self.pad_id] * bsz).long().view(bsz, -1)
                )  # [bsz,1]
                temp_pad = temp_pad.to(device)
                temp_input_ids = torch.cat([temp_pad, input_ids], dim=1)  # [B, L+1]
                bsz_idx, last_idx = [], []
                for i in range(bsz):
                    temp = temp_input_ids[i]
                    bsz_idx.append(i)
                    t = torch.nonzero(
                        temp == self.pad_id, as_tuple=True
                    )  # padding的位置
                    # t[0].max()是第一个非 padding的位置， + wanted_position是需要的位置
                    # 因为额外加了一层temp_pad,所以就不需要+1了
                    last_idx.append(t[0].max() + wanted_position)
    ...
    def forward(
        ...
        llm_embedding = self.reduce_hidden_states(
            input_ids=test_input_ids,
            attention_mask=test_attention_mask,
