...
def euclidean_distance_same_shape(x,y,do_sqrt=True):
    """
    Args:
        x: [B,D]
        y: [B,D]
    ...
    ret = torch.sum((x-y)**2,dim=1).view(-1,1)
    if do_sqrt:
        ret = torch.sqrt(ret)
    return ret


@Register(name="eva02_clip_for_triplet_euclidean")
class EVA02CLIPForTriplet(nn.Module):
    ...
    def forward(
        ...
        欧式距离的三元组损失
        """
        anchor_embed = self.encode_text(anchor_input_ids, normalize=False)  # [B, D]
        positive_embed = self.encode_image(positive_image, normalize=False)  # [B, D]
        negative_images = torch.cat(negative_images, dim=0)  # [N, C, H, W]
        negative_embeds = self.encode_image(negative_images,normalize=False)  # [N, D]
        B, D = anchor_embed.size()
        N = negative_embeds.size(0)
        dist_ap = euclidean_distance_same_shape(anchor_embed, positive_embed)  # [B,1]
        dist_ap = dist_ap.repeat(1, N)  # [B, N]
        dist_an = euclidean_distance(anchor_embed, negative_embeds)  # [B, N]
        # 共有B*N个三元组
    ...
    def inference_forward(self, image, text):
        ...
        image_embed = self.encode_image(image, normalize=False)
        text_embed = self.encode_text(text, normalize=False)
        euc_dist = euclidean_distance_same_shape(image_embed, text_embed).view(-1)
        score = euc_dist.detach().to(torch.float32).cpu().numpy().tolist()
        return None, score
