...
class DefaultTester:
    ...
    def test(self):
        for checkpoint_path in self.checkpoint_list:
            ...
            # cal metrics
            self.cal_metrics(save_path, save_path.replace("result.json", "metrics.json"))
    ...
    def cal_metrics(self, result_path, save_path):
        ...
        for m in self.metrics_config:
            ...
            logger.info(f"{result_path}=> {metrics_type}: {metrics_value}")
    ...
    def _create_dataloader(self, dataloader_config: dict):
        ...
        if max_steps > 0:
            self.dataloader = itertools.islice(
                self.dataloader, min(max_steps, self.dataloader.__len__())
            )
