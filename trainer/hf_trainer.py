...
def build_hf_trainer(config):
    ...
    training_args = hfparser.parse_dict(
        build_training_args_dict(config), allow_extra_keys=True
    )[0]
    trainer_cls = config.get("trainer_cls", None)
    ...
    def train(self):
        ...
        self.trainer._save(
            output_dir=os.path.join(
                ...
                f"checkpoint-{self.trainer.state.global_step}",
            )
        )
