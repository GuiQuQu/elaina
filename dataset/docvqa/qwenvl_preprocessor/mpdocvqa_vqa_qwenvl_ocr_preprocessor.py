...
@Register(name="mpdocvqa_vqa_qwenvl_ocr_preprocessor")
class MPDocVQAVqaQwenVLOCRPreprocessor(BasePreprocessor):
    def __init__(
        self,
        model_path,
    ...
    def transform_ocr2layout(self, ocr_path):
        ...
        layout, is_truncated = truncate_layout(layout, tokenizer=self.tokenizer, max_token_length=self.max_layout_length)
        if is_truncated:
    ...
    def preprocess(self, item):
        ...
        train_conversation, test_conversation = self.get_prompt(
            ...
            image_path=f"<img>{true_image_path}</img>",
            question=question,
            layout=layout,
        )
